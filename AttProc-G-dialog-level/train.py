...
numIterPerEpoch = int(math.ceil(dataloader.num_dialogues/args.batchSize))
...
model = Model(imgSize=args.imgFeatureSize,
              attImgSize=args.attImgFeatureSize, 
              attImgHeight=args.attImgFeatureHeight, 
              attImgWidth=args.attImgFeatureWidth,
              attVectSize=args.attVectSize,
              attTimes=args.attTimes,
...
with tf.Session() as sess:
  ...
  for epoch in range(1, args.numEpochs+1):
    ...
    for batch_id, (start, end) in enumerate(zip(range(0, dataloader.num_dialogues, args.batchSize), range(args.batchSize, dataloader.num_dialogues, args.batchSize))):
      batchImgFeat, batchAttImgFeat, batchQuestion, batchQuestionLength, batchCaption, batchCaptionLength, batchAnswer, batchAnswerLength, batchTarget = dataloader.getTrainBatch(idxs[start:end])
      input_feed = { model.image_feature_ph:batchImgFeat,
          model.attention_image_feature_ph:batchAttImgFeat,
