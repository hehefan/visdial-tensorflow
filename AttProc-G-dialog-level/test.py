...
with tf.Session() as sess:
  ...
  for step in range(start_step, 202000, args.ckptInterval):
    ...
    for batch_id, (start, end) in enumerate(zip(range(0, dataloader.num_dialogues, args.batchSize), range(args.batchSize, dataloader.num_dialogues, args.batchSize))):
      batchImgFeat, batchAttImgFeat, batchQuestion, batchQuestionLength, batchCaption, batchCaptionLength, batchAnswerID, batchOptions, batchOptionLengths = dataloader.getTestBatch(idxs[start:end])
