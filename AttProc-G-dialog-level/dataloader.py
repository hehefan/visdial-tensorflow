...
class DataLoader(object):
  ...
  def __init__(self, opt, subset):
    ...
    attImgFeats = np.array(imgFile['images_'+subset+'_7'])
  ...
  def getTrainBatch(self, batch_idx):
    batchImgFeat, batchAttImgFeat, batchQuestion, batchQuestionLength, batchCaption, batchCaptionLength, batchAnswer, batchAnswerLength, batchTarget = [], [], [], [], [], [], [], [], []
    for idx in batch_idx:
      batchImgFeat.append(self.imgFeats[idx])
      batchAttImgFeat.append(self.attImgFeats[idx])
    ...
    return np.array(batchImgFeat), np.array(batchAttImgFeat), np.array(batchQuestion), np.array(batchQuestionLength), np.array(batchCaption), np.array(batchCaptionLength), np.array(batchAnswer), np.array(batchAnswerLength), np.array(batchTarget)


  def getTestBatch(self, batch_idx):
    batchImgFeat, batchAttImgFeat, batchQuestion, batchQuestionLength, batchCaption, batchCaptionLength, batchAnswerID, batchOptions, batchOptionLengths = [], [], [], [], [], [], [], [], []
    for idx in batch_idx:
      batchImgFeat.append(self.imgFeats[idx])
      batchAttImgFeat.append(self.attImgFeats[idx])
    ...
    return np.array(batchImgFeat), np.array(batchAttImgFeat), np.array(batchQuestion), np.array(batchQuestionLength), np.array(batchCaption), np.array(batchCaptionLength), np.array(batchAnswerID), np.array(batchOptions), np.array(batchOptionLengths)
