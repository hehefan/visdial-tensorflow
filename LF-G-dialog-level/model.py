...
class Model(object):
  def __init__(self, imgSize, vocabSize, embedSize, use_lstm, rnnHiddenSize, rnnLayers, pad, start, end, 
    batch_size, learning_rate, learning_rate_decay_factor, min_learning_rate, training_steps_per_epoch, 
    ...
    for r in range(10):
      ...
      with tf.variable_scope('DecoderRNN', reuse=(r>0)) as varscope:
        if is_training:
          ...
          def loop_function(prev, _):
            ...
            prev_emb = embedding_ops.embedding_lookup(embedding, prev_symbol)
            self_answer_emb.append(prev_emb)
            return prev_emb
      ...
      if is_training:
        ...
        ans_word_probs.append(decoder_outputs)
    if is_training:
...
#model = Model(imgSize=1024, vocabSize=9999, embedSize=300, use_lstm=True, rnnHiddenSize=512, rnnLayers=2, start=1, batch_size=64, end=2, learning_rate=1e-3, learning_rate_decay_factor=0.99, min_learning_rate=1e-4, training_steps_per_epoch=10, is_training=True) 
