...
class DataLoader(object):
  ...
  def __init__(self, opt, subset):
    ...
    answer_targets = np.zeros((self.num_dialogues,10,21), dtype=np.int)
    for d in range(self.num_dialogues):
      for r in range(10):
        answer_targets[d][r] = np.insert(answers[d][r], answer_lengths[d][r], self.word2ind['<END>'])
    self.answer_targets = answer_targets

    option_target_list = np.zeros((option_list.shape[0],21), dtype=np.int)
    for o in range(option_list.shape[0]):
      option_target_list[o] = np.insert(option_list[o], option_lengths[o], self.word2ind['<END>'])
    
    self.options = options
    self.option_list = option_list
    self.option_target_list = option_target_list
    self.option_lengths = option_lengths

  def getTrainBatch(self, batch_idx):
    batchImgFeat, batchQuestion, batchQuestionLength, batchCaption, batchCaptionLength, batchAnswer, batchAnswerLength, batchAnswerTarget = [], [], [], [], [], [], [], []
    for idx in batch_idx:
      ...
      batchAnswerTarget.append(self.answer_targets[idx])

    return np.array(batchImgFeat), np.array(batchQuestion), np.array(batchQuestionLength), np.array(batchCaption), np.array(batchCaptionLength), np.array(batchAnswer), np.array(batchAnswerLength), np.array(batchAnswerTarget)


  def getTestBatch(self, batch_idx):
    batchImgFeat, batchQuestion, batchQuestionLength, batchCaption, batchCaptionLength, batchAnswer, batchAnswerLength, batchAnswerID, batchOptions, batchOptionTargets = [], [], [], [], [], [], [], [], [], []
    for idx in batch_idx:
      batchImgFeat.append(self.imgFeats[idx])
      ...
      option_targets = np.zeros(shape=(10, 100, 21), dtype=int)
      option_ids = self.options[idx]   # [10, 100]
      for r in range(10):
        for o in range(100):
          i = option_ids[r][o]
          options[r][o] = self.option_list[i]
          option_targets[r][o] = self.option_target_list[i]
      batchOptions.append(options)
      batchOptionTargets.append(option_targets)

    return np.array(batchImgFeat), np.array(batchQuestion), np.array(batchQuestionLength), np.array(batchCaption), np.array(batchCaptionLength), np.array(batchAnswer), np.array(batchAnswerLength), np.array(batchAnswerID), np.array(batchOptions), np.array(batchOptionTargets)
